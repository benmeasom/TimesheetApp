...
$(document).ready(function () {
    ...
    if (minDate == "True") {
        GetTimesheetDateList(GetFormatedDate(thisSunday), BindTimesheetPicker);
    }
    else {
    ...
    $("#ReviewWeekEndingDate").flatpickr({
        ...
        defaultDate: $("#WeekEndingDate").val() == '' ? '' : new Date($("#WeekEndingDate").val()),
        dateFormat: "d-m-Y",
    ...
    $(document).on("change", "#ReviewWeekEndingDate", function (e) {
        ...
        if (e.currentTarget.value != '') {
            $.ajax({
                ...
                url: getTimesheetByEndDate + "?endDate=" + e.currentTarget.value,
                success: function (result) {
...
function BindTimesheetPicker(result) {
    ...
    if (minDate == "True") {
        ...
        $("#TimesheetDate").flatpickr({
            defaultDate: between.length > 0 ? between[0] : null,
            dateFormat: "d-m-Y",
            enable: between,
            onReady(_, __, fp) {
                fp.calendarContainer.classList.add("timesheetDatepickerClass");
...
function GetTimesheetData(timesheetId, reviewWeekEndingDate) {
    $.ajax({
        type: "GET",
        url: getTimesheetDetailsByTimesheetId + "?timesheetId=" + timesheetId + "&endDate=" + reviewWeekEndingDate,
        /*data: formData,*/
        processData: false,
        contentType: false,
        cache: false,
...
function DisableTimesheetDates(value) {
    //Working js format with DD-MM-YYYY
    ////var da = moment("25-12-1995", "DD-MM-YYYY");
    ////da.toDate()
    var areaLable = GetAreaLabelDate(value);
    /*$("span.flatpickr-day.nextMonthDay.flatpickr-disabled[aria-label*='" + areaLable + "']").css("background-color", "red").css("color", "white");*/
